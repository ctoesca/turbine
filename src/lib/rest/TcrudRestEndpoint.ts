...
export class TcrudRestEndpoint extends TrestEndpoint {
  ...
  public callService(req, res, next, f, ...args ){
    ...
    .then( (result) => {
      ...
      res.send(result)
      //this.pool.release(service)
    })
    .catch( (err) => {
      //this.pool.destroy(service)
      service.free()
      next(err)
    })
  }
