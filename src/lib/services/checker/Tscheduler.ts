...
export class Tscheduler extends TeventDispatcher {
	
	scheduleTimer: Ttimer
	saveTimer: Ttimer
	saving: boolean = false
	logger
	daoClass: string = "Service"
	redisClient
	scheduling: number = 0
			...
			if (error){
				...
				this.redisClient.lrange("results", 0, len-1, function(error, results){
					...
					if (error){
						...
						{
							//this.logger.trace("results : NO ITEM TO SAVE"); 
							this.savingResults = false;
						}
						else{
			...
			this.redisClient.llen("queue", function(error, result){
				...
				if (!error)
				{
					...
					.then( function(result){
						...
						//this.config.maxScheduleSize = idealRate

						payload.data.idealRate = idealRate
						//this.config.maxScheduleSize = idealRate+ Math.round(idealRate/10)
