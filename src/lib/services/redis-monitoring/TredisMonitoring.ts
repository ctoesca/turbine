...
export class TredisMonitoring extends TbaseService {
    ...
    constructor(name, config) {
        ...
        this.timer = new Ttimer({ delay: this.config.checkInterval * 1000 });
        this.timer.on(Ttimer.ON_TIMER, this.check.bind(this), this);
    }
    flatify() {
    ...
    checkByNode(host: string, port: number, callback) {
