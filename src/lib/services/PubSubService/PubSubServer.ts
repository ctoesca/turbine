...
export class PubSubServer extends TbaseService {
    clients: any[] = [];
    ...
    constructor(name, server, config) {
        ...
        this._channelsManager = new ChannelsManager(this);
    ...
    onSockLog(severity: string, message) {
        if (severity == "error")
            console.log(message);
    }
    ...
    sendChannelEvent(type: string, channelName: string, DBClient) {
        var message = {
            type: 'channel_event',
            channel: channelName,
            payload: {
    ...
    onCleanClientsTimer(evt: Tevent) {
        var now = new Date().getTime() / 1000;
    ...
    onConnection(conn, req: express.Request) {
        ...
        if (!this.config.useSockjs)
            conn.id = uuid.v4();
        var client = new Client(this, conn, { useSockjs: this.config.useSockjs });
    ...
    getClientsByUsername(username: string) {
        var r = [];
        if (username != null)
            for (var i in this.clients) {
                var c = this.clients[i];
    ...
    onRedisPubSubMessage(channel: string, data) {
    ...
    sendToUsers(userNames: string[], messages) {
    ...
    processBeforeRequest(req: express.Request, res: express.Response, next: express.NextFunction) {
        return true;
    }
    initRoutes() {
        this.app.post('/topics/sendMessageToUsers', (req: express.Request, res: express.Response, next: express.NextFunction) => {
        ...
        this.app.post('/topics/publish', (req: express.Request, res: express.Response, next: express.NextFunction) => {
            if (!this.processBeforeRequest(req, res, next))
                return;
            var type = 'publish';
            var messages = req.body;
        ...
        this.app.get('/stat', (req: express.Request, res: express.Response, next: express.NextFunction) => {
            if (!this.processBeforeRequest(req, res, next))
                return;
            res.status(200).send({});
        });

        this.app.get('/getJson',  (req: express.Request, res: express.Response, next: express.NextFunction) => {
            if (!req["session"]) {
        ...
        this.app.get('/getClusterConnexions', (req: express.Request, res: express.Response, next: express.NextFunction) => {
            if (!this.processBeforeRequest(req, res, next))
                return;
