...
export class Channel extends TeventDispatcher {
    ...
    subscribeClient(client: Client, notifySubscribeEvents) {
    ...
    createSubscription(client: Client) {
        var sub = new Subscription(this.name, client);
    ...
    unsubscribeClient(client: Client) {
        var r = null;
        for (var i = 0; i < this.subscriptions.length; i++) {
    ...
    _removeSubscriptionById(id: string) {
        var r = null;
        for (var i = 0; i < this.subscriptions.length; i++) {
    ...
    sendChannelEvent(client: Client, type: string) {
        var message = {
            type: 'channel_event',
            channel: this.name,
            payload: {
    ...
    _onSubscriptionDestroy(e: Tevent) {
        this._removeSubscriptionById(e.currentTarget.id);
    }
    getSubscriptions() {
        return this.subscriptions;
    }
    getSubscription(client: Client) {
        var r = null;
        var clientId;
