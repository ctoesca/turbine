...
class TtasksManager extends TbaseService_1.TbaseService {
    constructor(name, server, config) {
        ...
        this.refreshTimer = new Ttimer_1.Ttimer({ delay: 20 * 1000 });
        this.refreshTimer.on(Ttimer_1.Ttimer.ON_TIMER, this.onRefreshTimer, this);
    }
    getDefaultConfig() {
        return {
            "active": true,
    ...
    removeTask(id) {
        if (typeof this.runningTasks[id] != "undefined") {
            ...
            if (this.fileExists(path))
                fs.unlinkSync(path);
            var evt = new Tevent_1.Tevent("TASK_END", task);
